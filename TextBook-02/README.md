
# AI거버넌스 - 관련 법과 국제표준 : 제2강 

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_1.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_2.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_3.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_4.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_5.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_6.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_7.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_8.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_9.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_10.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_11.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_12.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_13.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_14.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_15.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_16.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_17.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_18.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_19.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_20.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_21.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_22.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_23.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_24.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_25.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_26.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_27.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_28.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_29.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_30.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_31.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_32.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_33.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_34.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_35.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_36.PNG'> 

<br>



**인공지능 윤리원칙 분류 모형 및 윤리 측정지표 개발과 신뢰성에 관한 연구(소순주, 2023)**
인공지능 윤리 측정지표(EMI-AI) 전체 86개 지표<br>

| 윤리원칙 | 구성요소 | 윤리 측정지표 항목 | 항목코드 |
|---------|-----------|----------------------|-----------|
| 투명성 | 설명가능성 | 최종 사용자를 위한 투명성 | A01 |
| 투명성 | 설명가능성 | 사고조사자를 위한 투명성 | A02 |
| 투명성 | 설명가능성 | 변호사 및 전문가 증인을 위한 투명성 | A03 |
| 투명성 | 설명가능성 | 이용 범위 고지 투명성 | A04 |
| 투명성 | 설명가능성 | 이용자 데이터수집 투명성 | A05 |
| 투명성 | 설명가능성 | 안전 인증절차를 위한 투명성 | A06 |
| 투명성 | 설명가능성 | 대중 및 이해관계자를 위한 투명성 | A07 |
| 투명성 | 추적가능성 | 문제 분석을 위한 데이터 제공 | A08 |
| 투명성 | 추적가능성 | 제작 과정의 기록 보관 | A09 |
| 투명성 | 추적가능성 | 사고 발생 이력 보관 | A10 |
| 투명성 | 추적가능성 | 상시 운영 감시 | A11 |
| 투명성 | 추적가능성 | 정기 운영 감시 | A12 |
| 투명성 | 검증가능성 | 안전성 시험기준 적용 | A13 |
| 투명성 | 검증가능성 | 알고리즘의 투명성 보완 | A14 |
| 투명성 | 검증가능성 | 학습용 데이터의 신뢰성 | A15 |
| 투명성 | 검증가능성 | 학습용 데이터의 충분성 | A16 |
| 투명성 | 검증가능성 | 시험·인증 결과 제시 | A17 |
| 책무성 | 준법성 | 법·제도 준수 | B01 |
| 책무성 | 준법성 | 개인정보 제공 동의 획득 | B02 |
| 책무성 | 준법성 | 데이터 사용 사전 허가 획득 | B03 |
| 책무성 | 준법성 | 법적 근거 확보와 제시 | B04 |
| 책무성 | 준법성 | 사용 설명서의 기능 일치성 | B05 |
| 책무성 | 준법성 | 사용범위 법적 기준 준수 명문화 | B06 |
| 책무성 | 준법성 | 이용자 안전 우선 준수 | B07 |
| 책무성 | 준법성 | 신고자에 대한 법적 보호 | B08 |
| 책무성 | 준법성 | 이용자 요구사항 대응 | B09 |
| 책무성 | 책임식별성 | 설계자 책임 식별 | B10 |
| 책무성 | 책임식별성 | 제작자 책임 식별 | B11 |
| 책무성 | 책임식별성 | 운영자 책임 식별 | B12 |
| 책무성 | 책임식별성 | 이용자 책임 식별 | B13 |
| 책무성 | 책임식별성 | 개발자 책임 식별 | B14 |
| 공정성 | 알고리즘 차별성 | 의도적 차별 요소 제한 | C01 |
| 공정성 | 알고리즘 차별성 | 개인 민감정보 차별 제한 | C02 |
| 공정성 | 알고리즘 차별성 | 불공정한 알고리즘 | C03 |
| 공정성 | 알고리즘 차별성 | 인간의 생명 위협 방지 | C04 |
| 공정성 | 알고리즘 차별성 | 알고리즘의 의사결정 프로세스 제공 | C05 |
| 공정성 | 알고리즘 차별성 | 차별적 본의 방지 | C06 |
| 공정성 | 데이터 편향성 | 편향적 특성 제거 | C07 |
| 공정성 | 데이터 편향성 | 데이터의 사전검사 | C08 |
| 공정성 | 데이터 편향성 | 학습용 데이터의 수량 | C09 |
| 공정성 | 데이터 편향성 | 데이터의 유효성 점검 | C10 |
| 공정성 | 데이터 편향성 | 학습용 데이터의 편향관리 | C11 |
| 공정성 | 데이터 편향성 | 데이터 편향 방지 알고리즘 적용 | C12 |
| 공정성 | 데이터 편향성 | 데이터 분류 기준 정립 및 적용 | C13 |
| 공정성 | 데이터 편향성 | 데이터 관리의 확보 | C14 |
| 공정성 | 접근성 | 접근성 보장 | C15 |
| 공정성 | 접근성 | 보호자 사용자 지원 | C16 |
| 공정성 | 접근성 | 사용자의 접근 권한 방지 | C17 |
| 공정성 | 접근성 | 이용자의 쉬운 접근성 | C18 |
| 공정성 | 공공성 | 공공데이터 수집 공개 | C19 |
| 공정성 | 공공성 | 데이터 활용 제안 | C20 |
| 공정성 | 공공성 | 개인정보 공개 제한 | C21 |
| 공정성 | 공공성 | 사회발전에 기여하는 방향 배제 | C22 |
| 공정성 | 공공성 | 인간의 생명 위협 방지 | C23 |
| 공정성 | 공공성 | 이해관계자의 의견 반영 | C24 |
| 공정성 | 공공성 | 친환경 에너지 활용 | C25 |
| 공정성 | 공공성 | 폐기 절차 | C26 |
| 공정성 | 공공성 | 모든 인류에게 공정한 제공 | C27 |
| 통제성 | 제어가능성 | 정책적 통제권 이양 | D01 |
| 통제성 | 제어가능성 | 이용자 제어장치 제공 | D02 |
| 통제성 | 제어가능성 | 생명 위협 시 자동 종료 | D03 |
| 통제성 | 제어가능성 | 위험요소 탐지 및 제어 | D04 |
| 통제성 | 제어가능성 | 위험 발생 시 쉬운 접근성 제공 | D05 |
| 통제성 | 이용자 주도성 | 이용자 위험 회피 | D06 |
| 통제성 | 이용자 주도성 | 자율적 조작 | D07 |
| 통제성 | 이용자 주도성 | 이용자 데이터 선택적 제공 | D08 |
| 통제성 | 이용자 주도성 | 이용자 데이터 삭제 | D09 |
| 통제성 | 이용자 주도성 | 작업상태 제공 | D10 |
| 통제성 | 모니터링 | 위험 대응 상태 제공 | D11 |
| 통제성 | 모니터링 | 운영상황 상시 모니터링 | D12 |
| 안전성 | 견고성 | 인간의 생명 보호 우선 | E01 |
| 안전성 | 견고성 | 알고리즘 오류 확인 | E02 |
| 안전성 | 견고성 | 외부 공격에 대한 견고성 | E03 |
| 안전성 | 견고성 | 예측 결과의 동일성 | E04 |
| 안전성 | 견고성 | 학습용 데이터의 정확성 | E05 |
| 안전성 | 견고성 | 기능의 지속적 업그레이드 및 배포 | E06 |
| 안전성 | 견고성 | 기능 프로세스의 정확한 정의 | E07 |
| 안전성 | 보안성 | 보안성 인증 | E08 |
| 안전성 | 보안성 | 사용자 인증 | E09 |
| 안전성 | 보안성 | 상호작용 데이터 보안 | E10 |
| 안전성 | 보안성 | 비인가자 연결 방지 | E11 |
| 안전성 | 보안성 | 알고리즘 시큐어 코딩 | E12 |
| 안전성 | 보안성 | 이용 종료 시 정보 삭제 | E13 |
| 안전성 | 프라이버시 보호 | 개인정보보호 영향평가 | E14 |
| 안전성 | 프라이버시 보호 | 개인정보 수집 고지 | E15 |
| 안전성 | 프라이버시 보호 | 개인정보 암호화 | E16 |
| 안전성 | 프라이버시 보호 | 개인 민감정보 수집 차단 | E17 |
| 안전성 | 프라이버시 보호 | 개인정보 활용 | E18 |
| 안전성 | 프라이버시 보호 | 미동의 데이터 수집 방지 | E19 |
| 안전성 | 프라이버시 보호 | 데이터 주권 보장 | E20 |

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_37.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_38.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_39.PNG'> 

<br>

<img width ='900' height = '600' src = 'https://github.com/YangGuiBee/AIG/blob/main/TextBook-01/images/AIG02_40.PNG'> 

<br>


**인공지능 로봇의 위험성 유형별 측정지표 개발과 인공지능 윤리원칙 관계성 연구(송현경, 2024)**
인공지능 로봇 위험성 유형별 52개 측정지표<br>

| No. | 위험성 유형 | 하위 항목 | 지표 ID | 측정지표(요약) | 연관 윤리원칙 |
|---:|---|---|---|---|---|
| 1 | 신체(물리) | 운행·작동 이상 | A1-1 | 통신 장애로 안전 이벤트 감지/전파 지연 | 안전·통제 |
| 2 | 신체(물리) | 운행·작동 이상 | A1-2 | 센서/인식 오류 | 안전 |
| 3 | 신체(물리) | 운행·작동 이상 | A1-3 | 운동지능 제어 실패 | 안전 |
| 4 | 신체(물리) | 운행·작동 이상 | A1-4 | 기체 성능 저하 | 안전·책무 |
| 5 | 신체(물리) | 운행·작동 이상 | A1-5 | 프로그램 오류/오작동 | 안전·투명 |
| 6 | 신체(물리) | 운행·작동 이상 | A1-6 | 모션/균형 유지 실패 | 안전 |
| 7 | 신체(물리) | 운행·작동 이상 | A1-7 | 압력·힘 제어 실패 | 안전·통제 |
| 8 | 신체(물리) | 인간 해칠 판단 | A2-1 | 의도 파악 인식 오류 | 투명·안전 |
| 9 | 신체(물리) | 인간 해칠 판단 | A2-2 | 협동작업 규칙 판단 오류 | 안전·통제 |
| 10 | 신체(물리) | 인간 해칠 판단 | A2-3 | 사고 시 희생자 선택 착오 | 투명·안전 |
| 11 | 신체(물리) | 인간 해칠 판단 | A2-4 | 의료·돌봄 맥락 자료 오류 | 안전·책무 |
| 12 | 신체(물리) | 인간 해칠 판단 | A2-5 | 위험 지시 오해석 및 유해 행위 실행 | 안전·통제 |
| 13 | 권리 | 인격권 침해 | B1-1 | 대상 인식 판단 오류 | 공정·안전 |
| 14 | 권리 | 인격권 침해 | B1-2 | 음성 인식 오류로 모욕 응답 | 공정·안전 |
| 15 | 권리 | 인격권 침해 | B1-3 | 감정 피드백 오류 | 공정 |
| 16 | 권리 | 인격권 침해 | B1-4 | 도덕성 미검증 데이터 활용 | 공정·책무 |
| 17 | 권리 | 인격권 침해 | B1-5 | 멀티모달 융합 오류 | 공정·안전 |
| 18 | 권리 | 자유의사 방해 | B2-1 | 의사결정 유도(넛지/다크패턴) | 공정·책무 |
| 19 | 권리 | 자유의사 방해 | B2-2 | 여론 형성/심리 조작 | 공정·투명 |
| 20 | 권리 | 자유의사 방해 | B2-3 | 관리자 개입 차단/우회 | 통제·책무 |
| 21 | 권리 | 자유의사 방해 | B2-4 | 필터링 오류 | 안전·공정 |
| 22 | 권리 | 자유의사 방해 | B2-5 | 우월적 지위에 의한 심리 지배 | 공정·책무 |
| 23 | 권리 | 선택적 서비스/정보 | B3-1 | 차별적 정보 제공 | 공정 |
| 24 | 권리 | 선택적 서비스/정보 | B3-2 | 잘못된/유해 언어 사용 | 공정 |
| 25 | 권리 | 선택적 서비스/정보 | B3-3 | 개인정보 오남용/침해 | 안전·책무 |
| 26 | 권리 | 선택적 서비스/정보 | B3-4 | 선별적 정보 노출 | 공정·투명 |
| 27 | 권리 | 선택적 서비스/정보 | B3-5 | 부적합 콘텐츠 추천 | 안전·공정 |
| 28 | 재산 | 정보 입력·탈취 | C1-1 | 제어망 침범 | 안전·책무 |
| 29 | 재산 | 정보 입력·탈취 | C1-2 | 비인가 접근 허용 | 안전·통제 |
| 30 | 재산 | 정보 입력·탈취 | C1-3 | 악성코드/바이러스 감염 | 안전 |
| 31 | 재산 | 정보 입력·탈취 | C1-4 | 데이터 손실·변조 | 안전·책무 |
| 32 | 재산 | 정보 입력·탈취 | C1-5 | 수집 정보 무단 이용 | 책무·투명 |
| 33 | 재산 | 기계 결함·오작동 | C2-1 | 센서 상태 모니터링 실패 | 안전·통제 |
| 34 | 재산 | 기계 결함·오작동 | C2-2 | 구동부 문제 | 안전 |
| 35 | 재산 | 기계 결함·오작동 | C2-3 | 감속기/제어기 오류 | 안전 |
| 36 | 재산 | 기계 결함·오작동 | C2-4 | 정비 불량 또는 교체주기 경과 | 책무·안전 |
| 37 | 재산 | 기계 결함·오작동 | C2-5 | 시설물 충돌/파손 | 안전·책무 |
| 38 | 재산 | 기계 결함·오작동 | C2-6 | 과열·과전류 등 전기적 이상 | 안전 |
| 39 | 재산 | 잘못된 의사결정 | C3-1 | 재물 훼손 판단 오류 | 안전·책무 |
| 40 | 재산 | 잘못된 의사결정 | C3-2 | 인식지능 오류로 손실 발생 | 안전 |
| 41 | 재산 | 잘못된 의사결정 | C3-3 | 창작물 무단 복제·침해 | 책무·투명 |
| 42 | 재산 | 잘못된 의사결정 | C3-4 | 하자 상품/서비스 오추천 | 책무·투명 |
| 43 | 재산 | 잘못된 의사결정 | C3-5 | 안전거리 계산 오류 | 안전 |
| 44 | 재산 | 잘못된 의사결정 | C3-6 | 알고리즘 성능 부족 | 투명·책무 |
| 45 | 재산 | 잘못된 의사결정 | C3-7 | 판단지능 저하(노이즈 등) | 안전·투명 |
| 46 | 사회 | 데이터 문제 | D1-1 | 부적절/무단 데이터 수집 | 공정·책무 |
| 47 | 사회 | 데이터 문제 | D1-2 | 데이터 유출 | 안전·책무 |
| 48 | 사회 | 데이터 문제 | D1-3 | 허위·조작 정보 생성/확산 | 투명·책무 |
| 49 | 사회 | 법규 판단 오류 | D2-1 | 법률·규칙 혼동 | 책무 |
| 50 | 사회 | 법규 판단 오류 | D2-2 | 신호/차선 인식 오류 | 안전 |
| 51 | 사회 | 도로상 작동 중지 | D3-1 | 전력/연료 부족·통신 장애 | 안전·통제 |
| 52 | 사회 | 도로상 작동 중지 | D3-2 | 보행자·차량 통행 방해 | 안전·책무 |


